...
import { initInput, clearFrame, isConfirm, isAttack } from './engine/input.js';
import { initTouchControls, setTouchMode } from './engine/touch.js';
...
initInput();
initTouchControls();   // on-screen buttons (touch devices only)
initBGM('Piedad_BGM.mp3');

// Map game state → which touch buttons are shown. The level-up overlay hides
// the controls (cards are tapped directly) and play restores them automatically.
...
function setStage(num) {
  ...
  if (st.final) {
    ...
    setBGMTrack('Piedad_BGM.mp3', 0.45);
...
function updatePlaying(dt) {
  ...
  // Manual attack (touch Attack button). Auto-attack still runs; this only
  // fires the main weapon when it's already off cooldown, so the tap feels
  // responsive without ever exceeding the weapon's natural fire rate.
  if (isAttack()) combat.triggerManualAttack(player, enemies, bosses);
  ...
  if (combat.pendingCurseNotice) {
    ...
    stageBanner = { text: '⚠ Magic drained! Level & skills reduced ⚠', timer: 2.5 };
  }
...
function update(dt) {
  ...
  // Show the right touch buttons for the (possibly just-changed) state.
  setTouchMode(TOUCH_MODE_FOR[state] || 'hidden');
