// HUD: HP / Experience / Timer / Kill count / Gems (drawn on high-res overlay)
...
export function drawHUD(ctx, player, gameState, meta = {}) {
  ...
  // Stage + Difficulty (small line in top-center)
  if (meta.stage) {
  ...
  ctx.textAlign = 'right';
  ctx.fillText(player.unlockedWeapons.length + ' WPN', W - 6, 11);

  // ── Dynamic difficulty indicator (intensity) ──
  if (meta.intensity != null) {
    const it = meta.intensity;
    // Near 1.0 = green, high = red
    const t = Math.min(1, Math.max(0, (it - 0.7) / 2.0));
    const r = Math.floor(120 + t * 135), g = Math.floor(220 - t * 170);
    ctx.fillStyle = `rgb(${r},${g},90)`;
    ctx.font = `bold 9px ${UI_FONT}`;
    ctx.textAlign = 'right';
    ctx.fillText(`INT x${it.toFixed(2)}`, W - 6, 23);
  }

  // --- Revive (displayed near center) ---
  if (player.revives > 0) {
    ctx.fillStyle = '#ffee99';
    ctx.font = `bold 8px ${UI_FONT}`;
    ctx.textAlign = 'left';
    ctx.fillText(`Revive x${player.revives}`, 360, 11);
  }
