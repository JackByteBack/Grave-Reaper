...
export class CombatSystem {
  ...
  // Manual attack from the touch Attack button: fire the main weapon at the
  // nearest target if it's ready. Respects the weapon's own cooldown so it can
  // never out-pace the automatic firing — it just lets the player trigger it.
  triggerManualAttack(player, enemies, bosses) {
    if (player.dead) return;
    const weaponId = player.unlockedWeapons[0];
    const wDef = WEAPON_DEFS[weaponId];
    if (!wDef) return;
    if ((player.weaponCooldowns[weaponId] || 0) > 0) return;

    player.weaponCooldowns[weaponId] = wDef.cooldown;
    const allEnemies = [...enemies, ...bosses];
    if (wDef.type === 'strike')      this.fireStrike(wDef, player, allEnemies);
    else if (wDef.type === 'nova')   this.fireNova(wDef, player, allEnemies);
    else                             this.fireWeapon(wDef, player, allEnemies);
    if (player.triggerAttackAnim) player.triggerAttackAnim();
  }
